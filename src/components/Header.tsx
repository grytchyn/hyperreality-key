// ── HEADER — EN-only, no language switcher ──
    ...
    <header className="sticky top-0 z-50 w-full px-4 py-3"
      ...
      <div className="max-w-2xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          {showLevel && level && (
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-mono font-bold uppercase tracking-wider"
              style={{ background: `${levelColor}12`, border: `1px solid ${levelColor}30`, color: levelColor, boxShadow: `0 0 12px ${levelColor}12` }}>
          ...
          {totalScore !== undefined && totalScore > 0 && (
            <div className="flex items-center gap-1 px-2 py-1.5 rounded-lg text-[9px] font-mono font-bold tabular-nums"
      ...
      <div className="absolute bottom-0 left-0 right-0 h-[1px]"
        style={{ background: 'linear-gradient(90deg, transparent, color-mix(in srgb, var(--color-neon-purple) 30%, transparent), color-mix(in srgb, var(--color-neon-cyan) 30%, transparent), transparent)' }} />
    </header>
  )
}