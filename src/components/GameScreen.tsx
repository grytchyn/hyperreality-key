    ...
    <div className="relative overflow-hidden min-h-[100dvh]" ref={containerRef}
      ...
      <div className="max-w-2xl mx-auto p-3 sm:p-4 space-y-4 relative z-10 animate-fade-in-up">
        ...
        <div className="relative rounded-2xl overflow-hidden"
          style={{
            background: 'linear-gradient(180deg, color-mix(in srgb, var(--color-dark-card) 95%, transparent), color-mix(in srgb, var(--color-dark-card) 98%, transparent))',
          ...
          {/* Social bar */}
          <div className="px-4 py-2 flex items-center gap-4 text-[11px] border-b border-gray-800/30" style={{ color: 'var(--color-text-muted)' }}>
            <span>💬 12</span>
            <span>🔄 47</span>
            <span>❤️ 120</span>
          </div>
        ...
        <div className="rounded-2xl p-5"
            ...
            {post.choices.map((choice, idx) => {
              ...
              return (
                <button key={idx} onClick={() => handlePick(idx)}
                  ...
                  style={{
                    borderColor,
                    background: showResult && isCorrect ? 'color-mix(in srgb, var(--color-pixel-green) 10%, transparent)' : selected ? `${levelCfg.color}20` : 'rgba(255,255,255,0.02)',
                  ...
                  <span className="w-8 h-8 rounded-full flex items-center justify-center text-[12px] font-bold shrink-0"
                    style={{
                      background: showResult && isCorrect ? 'var(--color-pixel-green)' : selected && showResult ? 'var(--color-neon-red)' : selected ? levelCfg.color : 'rgba(255,255,255,0.06)',
          ...
          {chosenAnswer && (
            <div className="mt-4 rounded-xl p-3.5 text-xs leading-relaxed animate-fade-in-up"
              style={{
                background: feedback === 'correct' ? 'color-mix(in srgb, var(--color-pixel-green) 6%, transparent)' : 'color-mix(in srgb, var(--color-neon-red) 6%, transparent)',
            ...
            <button onClick={handleNext}
              ...
              style={{
                background: feedback === 'correct'
                  ...
                  : 'linear-gradient(135deg, var(--color-text-muted), color-mix(in srgb, var(--color-text-muted) 75%, #000))',
                color: '#fff',
                boxShadow: feedback === 'correct'
                  ? '0 4px 20px color-mix(in srgb, var(--color-pixel-green) 40%, transparent)'
                  : '0 4px 20px color-mix(in srgb, var(--color-text-muted) 30%, transparent)',
              }}>
              Next Level →
